	...
	<view class="swiperContent">
		...
		>
			<swiper-item class="swiper-item"  v-for="(item,index) in swiperList" :key="index" :class="(cardCur==index && displayMultipleItems ==1 && !vertical &&  !fullScreen)?'cur':''"  @tap="clickItem(index)">
				<view class="swiper-item-box" v-if="item[imageKey] && !item[videoKey]" :style="{transform: `scale(${scaleX},${scaleY})`}">
					<image :src="item[imageKey]" mode="aspectFill"></image>
				    <text class="swiper-text-box">
				    	<text class="swiperText" >{{item[textKey]}}</text>
				    	<text class="descText" >{{item[descKey]}}</text>
				    </text>
				</view>
